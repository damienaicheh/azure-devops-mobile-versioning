...
async function run() {
  try {
    ...
    if (tagResult.code !== 0) {
      ...
      task.error(
        "No tag found on this branch, please verify you have one in your remote repository."
      );
    ...
    setVariableOrDefault(
      NUMBER_OF_COMMITS_SINCE_TAG,
      numberOfCommitsSinceTag[0]
    );
    ...
    task.setResult(
      ...
      "Extract version from tag succeeded"
    );
  } catch (err) {
